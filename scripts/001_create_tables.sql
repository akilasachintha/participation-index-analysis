...
CREATE TABLE IF NOT EXISTS checklist_items (
  ...
  item_type TEXT NOT NULL CHECK (item_type IN ('analog', 'digital')),
  title TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS item_details (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  checklist_item_id UUID NOT NULL REFERENCES checklist_items(id) ON DELETE CASCADE UNIQUE,
  activity TEXT,
  image1_url TEXT,
  image2_url TEXT,
...
CREATE POLICY "Allow public read access to checklist_items" ON checklist_items FOR SELECT USING (true);
CREATE POLICY "Allow public insert access to checklist_items" ON checklist_items FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow public update access to checklist_items" ON checklist_items FOR UPDATE USING (true);
CREATE POLICY "Allow public delete access to checklist_items" ON checklist_items FOR DELETE USING (true);

CREATE POLICY "Allow public read access to item_details" ON item_details FOR SELECT USING (true);
CREATE POLICY "Allow public insert access to item_details" ON item_details FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow public update access to item_details" ON item_details FOR UPDATE USING (true);
CREATE POLICY "Allow public delete access to item_details" ON item_details FOR DELETE USING (true);
